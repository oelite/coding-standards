...
function resolveGuardPath(projectRoot: string): string {
  ...
  throw new Error(
    `oelite-guard.sh not found at ${candidate}. ` +
 ...
 */
function runGuard(guardPath: string, tool: string, args: Record<string, unknown>): void {
  if (process.env.OELITE_HUMAN === "1") return;
...
const OEliteGuard = async (_ctx: unknown) => ({
  "tool.execute.before": async (
    input: { tool: string },
    output: { args: Record<string, unknown> },
  ): Promise<void> => {
    if (!TOOLS_THAT_MUTATE.has(input.tool)) return;
